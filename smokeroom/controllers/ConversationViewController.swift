...
class ConversationViewController: UIViewController, UICollectionViewDelegateFlowLayout, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, referenceSizeForHeaderInSection section: Int) -> CGSize {
        ...
        let size = CGSize(width: approximateWidthOfBioTextView, height: 1000)
        let attributes = [kCTFontAttributeName: UIFont.systemFont(ofSize: 15)]
        let estimateFrame = NSString(string: (self.conversation?.text)!).boundingRect(with: size, options: .usesLineFragmentOrigin, attributes: attributes as [NSAttributedStringKey : Any], context: nil)
        // adjust if conversation has image
        if conversation.imageUrl == "" { // without
            return CGSize(width: view.frame.width, height: estimateFrame.height + 130)
        }
        else {
            // we add the width, because the image height is equal to width (square)
            return CGSize(width: view.frame.width, height: estimateFrame.height + 130 + view.frame.size.width)
        }
    }
    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        ...
        if (kind == UICollectionElementKindSectionHeader) {
            ...
            header.buzz.text = String(allComments.count)
            // load image if exists
            if conversation.imageUrl != "" {
                header.imageView.loadImageUsingCacheWithUrlString(conversation.imageUrl)
            }
                ...
                if error != nil { return }
                    ...
                    if let data = document?.data() {
                        ...
                        if self.conversation.ghostname {
                            ...
                            header.profile.image = UIImage(named: "logo")
                        }
                        else{
                            // download and set profile image
                            if user.profile_url != "" {
                                header.profile.loadImageUsingCacheWithUrlString(user.profile_url)
                            }
...
// button action -----------------------------------------------------
    ...
    @objc func commentButtonAction(_ sender:UIButton!){
        if(commentTextField.text == ""){
            ...
            // notify the creator of the post
            let notification = Notification(userid: conversation.userid, type: "Comment", type_id: conversationid, message: Helper.currentUser.name + " wrote in your smokeroom")
    ...
    @objc func ghostCommentButtonAction(_ sender:UIButton!){
        if(commentTextField.text == ""){
            ...
            // notify the creator of the conversation that you commented
            let notification = Notification(userid: conversation.userid, type: "Comment", type_id: conversationid, message: Helper.currentUser.name + " wrote in your smokeroom")
            self.db.collection("notifications").addDocument(data: notification.toAnyObject() as! [String: Any])
            // notify tagged users
            let tags = Helper.shared.extract_tags(text: comment.text)
...
// keyboard ---------------------------------------------------------------
    ...
    func loadImageAndFetchComments(_ urlString: String) {
...
//        image = UIImage()
//        if conversation.imageUrl != "" {
//            loadImageAndFetchComments(conversation.imageUrl)
//        }
//        else{
//            fetchComments()
//        }
